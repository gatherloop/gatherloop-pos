...
2. **The API must be served over HTTPS.** Pages is HTTPS-only, so a plaintext API origin is blocked as mixed content and the app simply will not work. `apps/api/docs/DEPLOY_NATIVE.md` documents TLS termination at the reverse proxy; this must be confirmed live before phase 4 merges.
...
Since the browser now calls the API cross-origin with no proxy, CORS carries real weight: `EnableCORS` already reflects the request origin and must also allow `X-Session-Id` (FR-1). The session travels as a header rather than a cookie, so no third-party-cookie restriction applies.
...
**Remaining prerequisites (verification, not decisions):**

1. **The API must answer over HTTPS** before phase 4 merges. GitHub Pages is HTTPS-only, so a plaintext API origin is blocked as mixed content and the app cannot work at all. `apps/api/docs/DEPLOY_NATIVE.md` documents TLS termination at the reverse proxy — this needs confirming against the live host, not just the doc.
2. **Confirm the public origin for QR printing** — `https://gatherloop.github.io/gatherloop-pos/order/...` as-is, or a custom domain via `CNAME`. Codes outlive printed stickers, so settling this before the first QR batch avoids a reprint. Changing it later means updating Vite's `base` and re-printing.
