...
**4. Cart (`/order/t/{tableCode}/cart`)** — line items with name, chosen options, note, quantity stepper, per-line subtotal, remove; a "add more items" link back to the menu; an order summary; and a sticky **Checkout** button. **Superseded (2026-09-08):** the cart-item edit modal is `/t/{code}/cart?item={id}`, a query param rather than the `/cart/items/{cartItemId}` path segment described elsewhere in this PRD. See D6 in `docs/trd-order-app-composition-and-ssr.md`.
