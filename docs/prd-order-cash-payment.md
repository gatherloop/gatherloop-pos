...
**Status:** Draft for review — three open questions, none blocking phase 1
...
**Option B — a separate `CASH_PAYMENT_EXPIRY_SECONDS` (default 900 s), enforced by a server-side sweeper. (Recommended)**
...
Wiring: `runNotificationSweeper` in `main.go:254` gains a third job and is renamed `runMaintenanceSweeper`. It keeps running on `KDS_DISPATCH_INTERVAL_SECONDS` (default 15 s) — a third env var for a job whose deadline is measured in minutes would be configuration nobody tunes, and 15 s of slack on a 15-minute window is noise. A `slog.Info` per expired payment carries the reference, the method and the transaction id.
...
|---|---|---|---|
| `CASH_PAYMENT_EXPIRY_SECONDS` | `apps/api/.env` | `900` | The walk-and-queue window (D5) |
...
| **D5** | Cash gets its **own expiry window**, `CASH_PAYMENT_EXPIRY_SECONDS`, defaulting to **900 s (15 minutes)** — not the 300 s of `DOKU_QRIS_EXPIRY_SECONDS`. | The acceptance criteria propose 5 minutes "like QRIS", but the two windows measure different things: DOKU's is a QR's validity, cash's is a walk downstairs plus a queue. Five minutes fails on any busy afternoon, and the failure mode is the worst available — cancelling an order for a guest who is standing at the till holding money. 15 minutes is one env var away from 5 if the operator disagrees, and expiry is cheap to get wrong in the guest's favour (the cart is preserved, re-ordering is one tap). **This is open question 1 — say the word and the default becomes 300.** |
...
| **D15** | The cashier's location is configuration (`NEXT_PUBLIC_ORDER_CASHIER_LOCATION`, default `Lantai 1`), not a string literal. | `tables.floor_number` already exists, so the venue already models floors. A hard-coded "Lantai 1" is a code change the day the till moves or a second one opens. |
...
| Risk | Mitigation |
|---|---|
| **An order is auto-cancelled while the guest is queueing at the till.** The worst failure in the feature: the cashier cannot find a transaction that has been soft-deleted, and the guest is standing there with money. | The 15-minute default (D5) is three times the QRIS window and set from the walk-and-queue reality, not from a QR's validity. The countdown is visible to the guest throughout. If it still happens, the cashier's recovery is the pre-existing one: key the order into the POS by hand from the guest's screen, which still shows the items. **A "cashier claims the order and stops the clock" mechanism is the first thing to build if this fires more than rarely** — it is Deferred, not rejected. |
...
| Not doing | Why |
|---|---|
| Switching the payment method on a live payment | The idempotency rule (QRIS D11) returns the existing payment whatever is asked. Cancel-and-remint is a state transition with its own races; the 15-minute window makes waiting cheap, and paying an existing QRIS reference in cash at the till already works. |
...
1. **Is 15 minutes the right cash window, or do you want the 5 minutes the acceptance criteria name?** D5 argues for 900 s; it is one env var, so the answer changes a default, not a design. *Blocks nothing — phase 2 ships with whichever number you name.*
2. **Should the cashier location be a single configured string ("Lantai 1"), or derived per table from `tables.floor_number`?** D15 takes the string, because one till serves both floors today. A per-floor mapping is additive. *Blocks nothing before phase 10.*
3. **Should an unpaid cash order print an order slip at the counter as well as buzzing the KDS?** `buildOrderSlipPayload` already exists and the counter has a printer. It would make the till's copy physical rather than a phone notification. Not designed here, and it would be a phase of its own. *Blocks nothing.*
