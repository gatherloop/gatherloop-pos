...
Paying doesn't end the guest's page — it starts the part that closes the loop. The same URL that showed the QR now shows a **preparing** screen: the guest's pickup number as the largest thing on it, a pulsing ring and an animated ellipsis proving the page is live, the table, and the item list — with no elapsed timer or countdown anywhere, deliberately. That screen also offers a **push notification** opt-in, so a guest who locks their phone and walks off still gets told the moment their order is ready — see [Guest Order-Ready Notifications](/sales/order-notifications). A barista finishing the order marks it **Ready** from the POS (see [Transactions](/sales/transactions)), and the guest's open page flips itself to a green **ready** screen within about ten seconds, telling them to collect at the counter with their number, whether or not they opted into the notification. Closing the tab raises no warning of any kind: the guest gets back to this same page from their [order history](/sales/order-history), one tap away on every order-app screen.
...
- Follow-up design doc: `docs/prd-order-web-push-notifications.md` — the push notification opt-in on the preparing screen; see [Guest Order-Ready Notifications](/sales/order-notifications)
