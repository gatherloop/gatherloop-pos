...
A customer sits down, scans the QR code printed on their table, and lands on a header naming the venue — its logo and name — with the table label plus its floor number underneath (e.g. "Meja 3 · Lantai 2"), so a guest can check both whose app they're in and that they're at the table the app thinks they are, even in a multi-floor venue. From there they browse categories, tap an item to pick its options (size, ice level, whatever the product defines), add a note, set a quantity, and add it to a cart. A floating bar keeps the running item count and total visible on every screen, genuinely pinned to the bottom of the viewport rather than the bottom of the page — including after the page has scrolled far enough for a phone browser's URL bar to collapse. A thin progress bar acknowledges every tap between screens, so a slow connection never leaves a button looking unresponsive. The cart survives closing the browser tab or reloading the page, so a customer who steps away and comes back later finds it exactly as they left it.
...
- **Branded on every screen** — a persistent header with the venue's logo and name sits above the table line on every screen, including the QR-resolution error states, so a guest never wonders whose app they're in.
...
- Customer app: `apps/order-web` — a Next.js app (Pages Router, same framework as the `apps/pos-web` POS), deployed on its own Vercel project; a permanent redirect shim on this docs site sends already-printed QR codes from the old GitHub Pages origin to it (docs/trd-order-app-nextjs-migration.md, D3)
- Route entry: `/t/{code}` (menu, `?product={id}` opens the item detail sheet), `/t/{code}/cart` (cart — recap, edit lines via `?item={id}`, and pay), `/t/{code}/status?ref={reference}` (payment/order status, see [Order Checkout (QRIS)](/sales/order-checkout)), each a server-rendered `getServerSideProps` page mounting one composition root — no separate routes for the overlays, so opening one is not a navigation (docs/trd-order-app-composition-and-ssr.md, D6). `/t/{code}/checkout` is a permanent redirect to `/t/{code}/cart` (`apps/order-web/next.config.js`), kept so a QR code or bookmark printed before that route was retired still lands somewhere useful.
- Frontend logic, reached through the `@gatherloop-pos/ui/order` entry point so the customer bundle never pulls in POS/Next code: `libs/ui/src/domain/usecases/{menuList,menuItemDetail,cart}.ts`, repositories in `libs/ui/src/data/{api,mock,session,url}`
- Anonymous session: `libs/ui/src/data/session/CookieSessionRepository.ts` (`gl_session_id` cookie + `localStorage`, sent as the `X-Session-Id` header) — resolved identically on the server and the client from the same cookie, so the menu and table render in the initial server response
...
- Follow-up design doc: `docs/prd-order-app-ux-round-2.md` — the route-change progress bar, the shell/footer fix behind "genuinely pinned", moving checkout onto this page (retiring `/checkout`), and the venue brand header covered above
