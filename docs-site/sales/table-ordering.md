...
- Route entry: `/t/{code}` (menu, `?product={id}` opens the item detail sheet), `/t/{code}/cart` (cart, `?item={id}` opens the edit-a-line modal), `/t/{code}/checkout`, each a server-rendered `getServerSideProps` page mounting one composition root — no separate routes for the overlays, so opening one is not a navigation (docs/trd-order-app-composition-and-ssr.md, D6)
