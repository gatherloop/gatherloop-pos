...
2. **Handler** — combines one or more controllers for a screen, maps domain entities to UI props, and wires cross-usecase effects (e.g. re-fetching the list after a delete succeeds). Example: [`TransactionListHandler`](https://github.com/gatherloop/gatherloop-pos/blob/main/libs/ui/src/presentation/screens/pos/TransactionListHandler.tsx).
3. **Screen** — pure Tamagui JSX driven entirely by props, with no business logic of its own. Example: [`TransactionListScreen`](https://github.com/gatherloop/gatherloop-pos/blob/main/libs/ui/src/presentation/screens/pos/TransactionListScreen.tsx).
4. **App composition** — the outermost piece, in [`libs/ui/src/app`](https://github.com/gatherloop/gatherloop-pos/tree/main/libs/ui/src/app), instantiates the real API repositories and usecases and renders the Handler. `app/` and `presentation/screens/` each split into a `pos/` and an `order/` folder, one per app (`libs/ui/src/app/pos`, `libs/ui/src/app/order`), so a screen or composition root belongs to exactly one app. This POS example is the exact component both `apps/pos-web` and `apps/pos-mobile` import via `@gatherloop-pos/ui/pos` — see [Cross-Platform (Web + Mobile)](/under-the-hood/cross-platform) for how the same composition serves both.

Both `apps/pos-web` and `apps/order-web` are Next.js apps (Pages Router) composed identically: a page file is nothing but a `getServerSideProps` that resolves request-scoped data (session, route params) server-side and a default export pointing at a composition root — no JSX, no local components, no client-only gates. `libs/ui/src/app/order` holds four composition roots and an `index.ts`, the same shape as `app/pos`: no provider, no context, no hook, no wrapper. See `docs/trd-order-app-composition-and-ssr.md` for how the order app converged on this from an earlier, client-only design.
