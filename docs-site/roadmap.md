...
- **Notifying kitchen/bar when an order-app transaction appears** — today it shows up in the existing POS list and order-slip print like any other transaction; nothing pushes to a kitchen display.
- **Notifying the customer when their order is ready** — the [order-status screen](/sales/order-checkout) ends at "sedang disiapkan"; there's no ready-for-pickup ping back to the guest.
- **More payment methods** — virtual account, e-wallet, through the same gateway port [order checkout](/sales/order-checkout) already has for QRIS.
- **Refunds and cancellation** — a POS-side flow needs to exist before an order-app payment can be refunded through the gateway.
