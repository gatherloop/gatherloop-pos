...
describe('AuthLoginHandler (kds)', () => {
  ...
  it('should navigate to "/device-setup" after successful login', async () => {
    const user = userEvent.setup();
    render(<AuthLoginHandler {...createProps()} />);

    await user.type(screen.getByRole('textbox', { name: 'Username' }), 'andi');
    await user.type(
    ...
    expect(mockRouterPush).toHaveBeenCalledWith('/device-setup');
  });
